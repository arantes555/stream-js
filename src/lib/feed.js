...
StreamFeed.prototype = {
  ...
  removeActivity: function (activityId, callback) {
     ...
     */
    const identifier = (activityId.foreignId) ? activityId.foreignId : activityId
    const params = {}
  ...
  follow: function (targetSlug, targetUserId, options, callback) {
    ...
    const target = targetSlug + ':' + targetUserId
  ...
  subscribe: function (callback) {
    ...
    if (!this.client.appId) {
      throw new errors.SiteError('Missing app id, which is needed to subscribe, use const client = stream.connect(key, secret, appId);')
    }
